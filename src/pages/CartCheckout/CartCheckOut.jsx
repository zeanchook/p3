...
export default function CartCheckOut()
{
  ...
  const style = {
    ...
    const orderDetails = cartStates && cartStates?.orderLine?.map((item,idx) => 
        {
        return(
        <ul role="list" className="divide-y divide-blue-200 hover:divide-pink-200" key={idx} 
        style={{width:"100%"}}>
          <p></p>
        <li  className="flex justify-between gap-x-20 py-5">
          <div className="flex min-w-0 gap-x-4">
            <img className="h-14 w-14 flex-none rounded-full bg-gray-50" src={item.product_id.picture} alt="" />
            <div className="min-w-0 flex-auto">
              <p className="text-sm font-semibold leading-6 text-gray-900">{item.product_id.title}</p>
              <p className="mt-1 truncate text-xs leading-5 text-gray-500">Unit Price: {item.product_id.price} | Qty {item.orderQty}
              </p>
            </div>
          </div>
          <div className="hidden shrink-0 sm:flex sm:flex-col sm:items-end" style={{display:"flex",alignItems:"center"}}>
            <p className="text-sm leading-6 text-gray-900">${(item.product_id.price  * item.orderQty).toFixed(2)}</p>
            {item !== "Delivered"? (
